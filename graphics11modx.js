...
$(function (){
	...
	var historie; // Array of arrays with livecells
	var running = false;
	var liferules = [[],[]];
	...
	$('#thetoroid').click( function(event) {
		...
		historie[steps] = livecells.slice();
		drawcells();
		updatedata();
	});
	...
	function drawgraph() {
		var ctx = graphcanvas.getContext('2d');
		var xPos = steps % graphcanvas.width;
		var yPos = graphcanvas.height - cellsalive * yscale;
		ctx.fillStyle = "rgb(128, 128, 0)";
	...
	function back1step(){
		if (steps > 0) {
			...
			steps -= 1;
			fadeall();
			livecells=historie[steps].slice();
			drawcells();
			updatedata();
		}
	}
